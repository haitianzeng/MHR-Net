...
def reprojection_loss(predicted, target):
    """
    Frobenius Norm with normalization
    """
    weights = torch.frobenius_norm(target, dim=[1, 2])
    loss_rpj = torch.frobenius_norm(predicted - target, dim=[1, 2])
    loss_rpj = (loss_rpj / weights).mean()
...
def normalized_error(predicted, target):
    """
    Normalized error (without Procrustes alignment) for evaluation
    """

    # expect predicted and target of [batch, n_points, 3]
    assert predicted.shape == target.shape and len(predicted.shape) == 3 and predicted.shape[2] == 3

    normalized_error = torch.norm(predicted - target, dim=(1, 2))
    normalized_error /= torch.norm(target, dim=(1, 2))
    normalized_error = torch.mean(normalized_error)

    return normalized_error.data.cpu().numpy()
...
def normalized_error_procrustes(predicted, target):
    """
    Normalized error with Procrustes alignment for evaluation
    """
    assert predicted.shape == target.shape, print(predicted.shape, target.shape)
    predicted = np.asarray(predicted.cpu())
    target = np.asarray(target.cpu())
    ...
    normalized_error = np.linalg.norm(predicted_aligned - target, axis=(1, 2))
    normalized_error /= np.linalg.norm(target, axis=(1, 2))
